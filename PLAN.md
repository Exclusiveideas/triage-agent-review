...
>
> **Post-implementation note:** items below were the live plan; **Status** lines were added after the work to map each item to what shipped vs deferred. Phases 0–3 + R-1 all shipped, Phase 4 mostly shipped (16 assertions across schema / dispatcher / sanitiser / `triageTicket` loop). T-6 case (b) and T-7 are still deferred — see NOTES.md "What I'd fix next" #2.
...
- **Status:** done — handler `Record` + `dispatchTool` with `is_error` listing valid tools. Unit test deferred to Phase 4.
...
- **Status:** done — `submit_triage` SDK tool, Zod-validated, `end_turn` without submission escalates as `error: "no_submission"`. Departed from spec by omitting `ticket_id` from the tool input (the agent already knows it at the call site). Unit test deferred to Phase 4.
...
- **Status:** done — delimiters + system-prompt clause + small `escapeDelimiterTags` sanitiser. Live `tkt_1007` run deferred — listed as #1 in NOTES "What I'd fix next" because it's the highest-information remaining test.
...
- **Test**: N/A — eyeball.
- **Status:** done — JSON line per call to stdout (status lines moved to stderr). `...response.usage` spread surfaces P3-2's cache fields automatically.
...
- **Status:** done — `await Promise.all(toolUses.map(dispatchTool))`. Buys nothing under sync mock handlers; the seam matters once `lookup_customer` becomes a real DB call. Unit test deferred to Phase 4.
...
- **Status:** done — two `cache_control` breakpoints (system + last tool entry). Switched to `client.beta.promptCaching.messages.create` because `@anthropic-ai/sdk@0.30.1` only exposes prompt caching under the beta namespace. Verification via P2-3's log; deferred until the live run (NOTES #1).
...
### R-1 — Extract `failureResult` helper for the four error-path TriageResult literals
- **Why**: by end of Phase 3 there were four near-identical `TriageResult` literals (`max_iterations_exceeded`, `stop_reason:*`, `no_submission`, `triage_threw:*`) all carrying the same placeholder `priority: "high"` for the open product question (P1-3, P3-3). One point of change once the team picks a policy; also closes the last `Record<string, any>` hole on the Phase 2/3-touched surface.
...
- **Status:** done — first audit shipped as `docs(N-1)`; second pass tightened per-fix entries to 1-2 sentences, filled in "What I found", and rewrote the voice.
